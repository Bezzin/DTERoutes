...
fs.writeFileSync(
  ERROR_LOG_FILE,
  `Batch Processing Error Log - ${new Date().toISOString()}\n${'='.repeat(
    60,
  )}\n\n`,
);
...
async function addTestCentre(config) {
  ...
  const {error: insertError} = await supabase.from('test_centers').insert({
    id: id,
    name: name,
    city: city,
    postcode: postcode,
...
async function processGpxFile(gpxPath, testCentreConfig) {
  ...
  if (!routeNumberMatch) {
    throw new Error(
      `Could not extract route number from filename: ${filename}`,
    );
  }
  ...
  const geojsonPath = path.join(
    TEMP_DIR,
    `${testCentreConfig.id}_route_${routeNumber}.geojson`,
  );
  const processedPath = path.join(
    TEMP_DIR,
    `${testCentreConfig.id}_route_${routeNumber}_processed.json`,
  );
...
async function processTestCentre(folderName, config) {
  ...
  if (!config.skip_test_centre_creation) {
    try {
      ...
      logError(
        `Failed to create test centre ${config.id}: ${error.message}\n${error.stack}`,
      );
...
async function main() {
  ...
  for (const [folderName, config] of Object.entries(TEST_CENTRE_CONFIG)) {
    ...
    try {
      ...
      logError(
        `Fatal error for ${config.id}: ${error.message}\n${error.stack}`,
      );
  ...
  for (const [testCentreId, result] of Object.entries(results)) {
    const config = Object.values(TEST_CENTRE_CONFIG).find(
      c => c.id === testCentreId,
    );
