...
async function seedRoute(processedFile, testCenterId) {
  ...
  console.log(`   Duration: ${originalFeature.properties.estimated_duration_mins} mins`);
  console.log(`   GPS Points: ${originalFeature.properties.point_count}`);
  console.log(`   Navigation Points: ${mapboxRoute.geometry.coordinates.length}`);
  ...
  if (testCenterError || !testCenter) {
    throw new Error(`Test center not found: ${testCenterId}. Run supabase_schema.sql first!`);
  }
  ...
  if (existingRoute) {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        estimated_duration_mins: originalFeature.properties.estimated_duration_mins,
        difficulty: 'moderate',
...
async function main() {
  ...
  if (args.length < 2) {
    ...
    console.error('\nExample:');
    console.error('  node seed_database.js ./stoke_route_1_processed.json stoke-on-trent-newcastle-under-lyme\n');
    process.exit(1);
  }
